...
## 摘要
 ...
 本规范描述了一种优化的超文本传输协议(HTTP).HTTP/2通过引进报头字段压缩以及多路复用来更有效利用网络资源、减少感知延迟。另外同样介绍了服务器推送规范。
...
特别是，HTTP/1.0只允许在一个给定的连接上建立一个请求。HTTP/1.1管道只部分解决了请求并发和报头阻塞的问题。因此客户端需要通过多次请求服务器以减少延迟。
...
此外，HTTP/1.1的报头字段经常重复和冗长，其中，为了产生更多或更大的网络数据包，可能会为了快速填充导致小的初始TCP窗口堵塞，这可能在多个请求建立在一个新的TCP连接时导致过度的延迟。
...
本协议通过定义一个基础连接的HTTP的语义优化的映射来解决这些问题。 具体地，它允许请求和响应消息建立在同一连接上，并使用高效率编码的HTTP报头字段。 它还允许请求的优先级，让更多的重要的请求更快速的完成，进一步提高了性能。
...
HTTP/2 提供了对HTTP语义的传输优化。HTTP/2支持所有HTTP／1.1的核心特征，并且在某些方面更高效。
...
请求多路复用是通过实现多个HTTP请求响应分配在一个流上实现的。流在很大程度上是相互独立的，因此一个请求上的阻塞或终止并不会影响其他请求的处理。
...
帧包含的HTTP报头字段是被压缩的。HTTP请求有可能是高度冗余的，因此压缩能显著减少请求和响应的大小。
...
文档中出现的关键字“`必须`”,“`绝对不能`”，“`要求`”，“`应`”，“`不应`”，“`应该`”，“`不应该`”，“`建议`”，“`或许`”及“`可选`”可在此找到解释RFC2119
...
所有的数值都是按网络字节顺序。除非有另外说明，数值是无符号的。按情况提供十进制或十六进制的文本值。十六进制以前缀0x来区分。
...
HTTP/2使用与HTTP/1.1相同的"http" and "https" 资源标识符(URI)。使用相同的默认端口："http" 的80端口及“https”的443端口。因此，实现对例如`http://example.org/foo`或`https://example.com/bar`目标资源的URI请求处理需要首先确定上游服务端(当前客户端希望建立连接的对等端)是否支持HTTP/2。
...
这意味着检测“http” 及“https” 的 URIs 是否支持HTTP/2的方法是不一样的。检测"http"  URIs 在章节3.2中描述。检测"https" URIs 在章节3.3中描述。
...
用到"h2" 或者 "h2c" 表明使用文档中电议的传输、安全、帧及语义化消息。
...
客户端无法预知是否支持HTTP/2.0 的情况下使用HTTP升级机制发起“http” URI请求([RFC7230] 章节6.7)。客户端发起一个http1.1请求，其中包含识别HTTP/2的升级头部字段与h2c令牌。HTTP/1.1必须包含一个确切的HTTP2-Settings中的头部字段。
...
如果一个请求的并发后续请求是重要的，那么可以使用一个小的请求来执行升级到HTTP/2,仅消耗一个额外的往返成本。
...
服务端必须忽略升级报头字段中的“h2”令牌。包含“h2” 令牌的基于TLS实现的HTTP/2,协商方法在章节3.3中定义。
...
这个HTTP2-Settings头部字段的内容是设置(SETTINGS)帧的有效载体，使用base64url字符编码(URL及文件名安全的Base64编码，编码描述在[RFC4648] 章节5中。) ABNF产品中对token68的定义在[RFC7235] 章节2.1中。
...
使用TLS的HTTP/2 使用"h2"程序令牌。“h2c”令牌绝对不能由客户端或者选定的服务端发送。
...
(字符串PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n)。这个序列后跟着一个设置帧。客户端在收到101转换协议响应(升级成功指示)后马上发送客户端连接序言，或者作为TLS连接的第一个应用数据字节。如果在对服务器支持HTTP/2具有先验知识的情况下启动HTTP/2连接，客户端连接序言在连接建立后就发送。
  ...
  客户端连接序言是这样选择的：大部分的HTTP/1.1或者HTTP/1.0服务端以及中介端不试图进一步处理帧。注意这并不能解决TALKING中提到的问题。
...
如果任何一个端点没有以一个有效的连接序言开头，客户端和服务端必须终止TCP连接。如果端点并没有使用HTTP/2此时超时GOAWAY 帧(章节6.8)可能可以省略。
...
如果一个帧大小超过任何设定的限制，或者太小无法包含强制的帧数据，这个端点必须发送一个帧大小错误FRAME_SIZE_ERROR。如果帧大小错误可能修改整个连接状态，必须作为一个ConnectionErrorHandler连接错误处理；这包括与0流一起的携带HeaderBlock头部块(即报文头HEADERS，推送许可PUSH_PROMISE和延续CONTINUATION)、设置SETTINGS以及任何窗口更新WINDOW_UPDATE的帧。
...
HTTP/2报文头部字段是包含一个或多个相关意义的键值对。他们在HTTP请求响应消息及服务器推送操作(见章节8.2)中使用。
...
报头压缩是有状态的，整个连接中使用同个压缩上下文。每个报头区块作为离散的单元处理。报头区块必须作为一个连续的帧序列传输，没有任何类型或任何其他流的交错帧。一个报头HEADERS或者延续CONTINUATION帧序列的最后一帧必须有头部终止标记设置。推送约定PUSH_PROMISE或者延续CONTINUATION帧序列的最后一帧必须具有头部终止标记设置。
...
报头区块必须被报头HEADERS、推送约定PUSH_PROMISE或延续CONTINUATION的有效载体发送，因为这些帧中携带了能被接收端修改的压缩上下文数据。端点在接收报头HEADERS、推送约定PUSH_PROMISE或延续CONTINUATION帧时必须重新组装报头区块并且执行解压缩，即便这些帧将被废弃。如何不能重建头部区间，接收端必须终止连接并报解压缩错误COMPRESSION_ERROR的ConnectionErrorHandler连接错误(章节5.4.1)。
...
流是一个独立的，客户端和服务端在HTTP/2连接下交换报头HEADERS和数据DATA帧的双向序列。
...
下列传输在这种状态下是有效的：
 ...
 - 发送一个推送约定PUSH_PROMISE帧标记相关的流后续再使用。保留流状态将转换为“预留(本地)”。
 - 接收一个推送约定PUSH_PROMISE帧标记相关的流为远程端点预留的流。这些流的状态变成“预留(远程)”
...
在“预留(本地)”状态的是已经被承诺发送推送约定PUSH_PROMISE帧的流。一个推送约定PUSH_PROMISE帧通过使一个流与一个由远端对等端初始化的打开的流相关联来保留一个空闲流。 
...
在这种状态下一个优先级PRIORITY帧可能被接收。接收到任何报头HEADERS帧、RST_STREAM帧或者优先级PRIORITY帧以外的帧都将被认为是类型为协议错误PROTOCOL_ERROR的ConnectionErrorHandler连接错误(章节5.4.1)。
...
接收任何RST_STREAM 和优先级PRIORITY以外的帧必须作为类型为 协议错误PROTOCOL_ERROR的ConnectionErrorHandler连接错误(章节5.4.1)来处理。
...
这种状态下，WINDOW_UPDATE, PRIORITY, or RST_STREAM帧可以在带有END_STREAM标记的数据DATA或者报头HEADERS帧发送一小段时间内被接收。在远端对等端接收并处理带有END_STREAM标记的帧之前，可以发送任意这几种帧。在这种状态下终端必须忽略接收到的WINDOW_UPDATE,PRIORITY, 或 RST_STREAM帧，但终端也可以当作协议错误PROTOCOL_ERROR或者ConnectionErrorHandler连接错误(章节5.4.1)来处理。
...
关闭的流上可以发送优先级帧用来对依赖当前关闭流的流进行优先级重排序。终端应该处理优先级帧，虽然在该流已经从依赖树(章节5.3.4)中移除时可以忽略。
...
如果流在发送RST_STREAM帧转换到这种状态，接收到RST_STREAM的对等端可能已经发送或者队列中准备发送无法取消的帧。终端必须忽略从已经发送RST_STREAM帧的流接收到的帧。终端可以选择设置忽略帧的超时时间并在超过限制后作为错误处理。
...
终端可能在发送RST_STREAM之后收PUSH_PROMISE帧。即便相关的流已经被重置，PUSH_PROMISE也能使流变成“预留”状态。因此，需要RST_STREAM来关闭一个不想要的被承诺流。
...
本文档中没有明确说明的地方，具体实现时接收描述状态中没有明确许可的信息都应作为类型为协议错误PROTOCOL_ERROR的ConnectionErrorHandler连接错误(章节5.4.1)来处理。
...
一个新建立的流标识符必须数值大于任何终端已经打开或者保留的流标识符。规则适用于使用HEADERS帧打开的流以及使用PUSH_PROMISE保留的流。终端收到不规范的流标识符必须响应一个类型为 PROTOCOL_ERROR的ConnectionErrorHandler连接错误connection error。
...
新的流标识符第一次被使用时将隐式关闭所有处于“空闲”状态下，可能已经被对等端初始化而且流标识符数字小于新标识符的流。例如，一个客户端发送一个流7的HEADERS帧，那么在流7发送或者接收帧后从没有发送帧的流5将转换为“关闭”状态。
...
流标识符不能被重复使用。生存期长的连接可能导致流标识符可用范围耗尽。客户端不能新建流标识符时可以针对新流建立一个新的连接。服务端不能新建流标识符时可以发送一个超时强制客户端对新的流使用新的连接。
...
对等端可以使用SETTINGS帧里面的SETTINGS_MAX_CONCURRENT_STREAMS参数来限制流的并发量。最大并发流设置(章节6.5.2)仅适用于终端并且只对接收到此设置的对等端有效。也就是说：客户端可以指定服务端能启动的流最大并发量，而且服务端能指定客户端能启动的流最大并发量。终端绝对不能超过对等端设置的限制。
...
每个流可以显式依赖其他流。包含一个依赖表示分配资源给识别的流而不是所依赖的流的偏好设置。
...
依赖其他流的流逝一个有依赖流。受依赖的流是父节点流。依赖的流如果当前不在树中——例如处于“空闲”状态的流——流将会被赋予一个默认的优先级(章节 5.3.5)。
...
专用标志允许插入一个新的层级的依赖。专用标志导致插入的流成为其父节点流唯一的子节点流，是其他依赖流变成依赖此优先流。在前面这个例子中，如果流D是用专业标志在来创建依赖流A的，那么将导致流D成为了B和C的依赖父节点流。
...
具有相同父节点的流应该根据权重比例来分配资源。因此，如果流B依赖流A的权重是4，流C依赖流A的权重是12，那么如果流A上不会有进展了，流B理论上将获取到相对于流C资源的三分之一。
...
流的优先级是通过使用优先级帧改变的。设置一个依赖将导致流依赖某个识别的父节点流。
...
当流从依赖树中移走后，依赖它的子流可以转移变成依赖被关闭流的父节点流。新的依赖的权重将根据关闭流的权重以及流自身的权重重新计算。
...
从依赖树中移除的流导致某些优先级信息丢失。资源在具有相同父流的流之间共享，这意味着如果这个集合中的某个流关闭或者阻塞，任何空闲容量将分配给最近的邻居流。然而，如果子流的共有依赖被从树中移除，这些子流将于上一层的流共享资源。
...
例如，假定A流和B流共有同个父依赖，且C和D流都依赖A流。在A流移除之前，如果A和D流都无法继续进行，那么C流就会接收所有分配给流A的资源。如果流A从树中移除，流A的权重将分配给流C和流D。如果流D依旧无法进行，将导致流C获取到的资源比例变少。对于同等的初始权重，流C获取到三分之一而不是二分之一的可用资源。
...
这可能增加终端的负担，因此这种状态可能被限制。终端可能会对跟踪状态的关闭的流的个数使用一个固定的上限来限制状态暴露。终端可能根据负荷来决定保留的额外的状态的数目；在高负荷下，可以丢弃额外的优先级状态来限制资源的任务。在极端情况下，终端甚至可以丢弃激活或者保留状态流的优先级信息。如果使用了固定的限制，终端应当至少保持跟SETTINGS_MAX_CONCURRENT_STREAMS设置一样大小的流。
...
发送流错误的终端应当首先发送一个超时GOAWAY帧(章节6.8)，并带有最近的一个成功从对等端接收帧的流的标识符。GOAWAY超时帧包含链接终端的错误码。发送GOAWAY后，终端必须关闭TCP连接。
...
超时GOAWAY帧有可能不被接收端有效接收。在连接错误事件中，超时GOAWAY帧是尝试跟对等端通信告知连接终止原因的最佳实践。
...
具体实现必须忽略扩展协议元素中未知或者不支持的值。实现必须丢弃包含未知或者不支持的类型的帧。这意味着任意这些扩展点能被扩展安全的使用，不许事先安排或协商。
...
然而，可能导致现有协议元素语言改变的扩展必须经过协商后才能使用。例如，一个修改报头帧布局的扩展使用之前必须收到对等端发送的允许使用的信号。在这种情况下，经过修订的布局生效的时候也可能有必要进行协商。注意对待任意数据帧以外的帧作为受流量控制的修改也是语义上的修改，也必须在通过协商才能完成。
...
填充字节的总数由Pad High的值乘以256加上Pad Low 的值决定。二者默认值均为0。如果填充物的大小大于帧有效载荷的大小，接收端必须作为类型为协议错误的连接错误(章节5.4.1)处理。
...
优先级帧(type=0x2)明确了发送者建议的流的优先级(章节5.3)。它可以任意时间在存在的流中发送。这个使存在的流有了优先级次序。
...
如果设置参数的发送端没有在一定时间内收到认可的响应，它可以发出一个类型为设置超时的连接错误(章节5.4.1)。
...
推送帧(type=0x5)用来在流发送者准备发送流之前告知对等端。推送帧包含了终端准备创建的长流的31位无符号标记以及提供附加上下文的报头的集合。章节8.2详细描述了推送帧的使用。
...
由于PUSH_PROMISE保留了一个流、忽略一个PUSH_PROMISE 帧都会导致流状态变得不确定。接收端接收到流状态不是“打开”或者“半封闭(本地)”的流的推送帧必须作为类型为协议错误的连接错误(章节5.4.1)处理。相似的，接收端必须对在一个非法标示(章节5.1.1)的流(即流的标识当前不在空闲状态)上建立的推送承诺作为类型为协议错误的连接错误(章节5.4.1)处理。
...
延续帧(type=0x9)用来延续一个报头区块(章节4.3)碎片序列。在现有流上可以发送任意数量的延续帧，只要相同流上的前一阵是报头帧、推送承诺帧或者不带有END_HEADERS标记的延续帧。
...
All HTTP/2 requests MUST include exactly one valid value for the :method, :scheme, and :path header fields, unless this is a CONNECT request (Section 8.3). An HTTP request that omits mandatory header fields is malformed (Section 8.1.2.5).

所有的HTTP/2请求都必须在:method、:scheme和:path字段上有准确的值，除非是一个连接请求。缺少基本报头字段的HTTP请求是不规范(章节8.1.2.5)的。
...
一个单一的:status字段定义用来携带HTTP状态码字段(见xxx)。这个报头字段必须包含在所有响应中，除非响应是不规范(章节8.1.2.5)的。
...
客户端不能推送。因此，服务端收到客户端的PUSH_PROMISE帧必须作为连接错误(章节5.4.1)处理。客户端不许拒绝任何尝试修改SETTINGS_ENABLE_PUSH设置值为0以外的值，并将这消息作为类型为协议错误的连接错误(章节5.4.1)处理。
...
服务端只能推送被缓存的响应(见RFC7234,章节3);被承诺的请求必须是安全的(见RFC7231,章节4.2.1)，而且绝对不能包含一个请求主体。
...
发送PUSH_PROMISE帧后，服务端可以开始传送推送响应作为一个响应(章节8.1.2.2)，在由服务端初始化的使用被承诺的流标识符的流上传送。服务端使用这些流传送一个HTTP响应，使用HttpSequence中定义的相同的帧序列。在初始化报头帧发送后，流对客户端(章节5.1)变为“半封闭”状态。
...
支持CONNECT的端口建立一个TCP连接到服务器，如:authority报头字段中定义。一旦连接建立成功，代理发送一个报头帧包含一个2xx序列状态码到客户端，如RFC7231中定义，章节4.3.6。
...
对于https资源，连接复用还另外需要一个对于URI中的主机已经验证过的证书。这是多subjectAltName熟悉或者使用通配符的名称的服务端证书的使用。例如，一个带有*.example.com的subjectAltName的证书将允许a.example.com和b.example.com.使用同个连接。
...
在某些部署环境中，多个源的连接复用可能导致请求被指向错误的源服务器。例如，代理可能使用TLS服务端名称指示(SNI)[TLS-EXT]扩展选择一个源服务器来执行TLS终止。这意味着可能导致客户端未像预期那样发送机密信息到请求的服务器，尽管服务端有验证的权限证书。
...
421响应默认是缓存住的；即除非被定义的方法或显示的缓存控制另有说明(RFC7234, 章节4.2.2)。
...
TLS实现必须禁用协商。终端必须将TLS协商作为类型为协议错误的连接错误(章节5.4.1)处理。需要注意的是由于密码套件可以加密的消息的数据限制，禁用协商可到导致长期连接变成不可用。
...
实现中对于HTTP/2不应该在即没有使用符合需求的加密套件协商的情况下又不进行TLS协商。由于实施的限制，不可能基于所有需求来使TLS协商失败。终端必须终止不符合TLS最小需求的TLS会话上建立的HTTP/2连接，并作为类型为INADEQUATE_SECURITY的连接错误(章节5.4.1)处理。
...
至于这个可观察到的行为的差异，他们可以作为特定的客户端的指纹的基础，见HTML5 章节1.8中定义。
...
本文档建立了帧类型、设置和错误码的记录。这些新的记录一收录到新的"Hypertext Transfer Protocol (HTTP) 2 Parameters"章节中。
...
本文档记录了HTTP总会难过的PRI方法，来避免与连接序言(章节3.5)冲突。
...
本文档建立了HTTP/2帧类型码的注册表。"HTTP/2 Frame Type"注册表管理一个8位的空间。"HTTP/2 Frame Type"注册操作在"IETF Review"或者"IESG Approval"政策下设置0x00 到 0xef的值， 0xf0 到 0xff的值是预留给实验用的。
